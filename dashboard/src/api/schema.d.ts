...
export interface paths {
    ...
    "/admin/plugins": {
        ...
        /**
         * Plugin status
         * @description Every tracked plugin and its lifecycle state, including plugins QUARANTINED at load (e.g. invalid config) — so an organizer can see WHY a plugin isn't working without reading boot logs. Reasons never contain a secret config value.
         */
        get: operations["adminListPlugins"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
...
export interface components {
    schemas: {
        ...
        /** @description Tracked plugins and their lifecycle state. */
        AdminPluginList: {
            plugins: components["schemas"]["AdminPluginStatus"][];
        };
        /** @description One plugin's lifecycle state. Reason is admin-facing and never carries a secret config value. */
        AdminPluginStatus: {
            /** @description Manifest name / registry key. */
            name: string;
            /** @description Plugin type: scoring | notification | challenge_type | auth. */
            type: string;
            /** @description Lifecycle state: discovered | launching | ready | unhealthy | restarting | failed | draining | stopped. */
            state: string;
            /** @description Why it is in this state when that needs explaining (e.g. a quarantine cause). Redacted of secret values. */
            reason?: string;
        };
    };
    responses: {
