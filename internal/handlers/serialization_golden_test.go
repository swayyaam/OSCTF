...
var objectKeys = map[string]bool{"container_env": true, "type_config": true}
...
func TestGoldenFull(t *testing.T) {
	cases := []struct {
		...
		{"challenge_admin.full", apigen.ChallengeAdmin{
			...
			Type: "standard", TypeConfig: map[string]string{"pattern": "x"},
		...
		{"challenge_admin_page.full", apigen.ChallengeAdminPage{
			Items: []apigen.ChallengeAdmin{{Id: uidN(3), Slug: "sqli", Title: "T", Category: apigen.Category("web"), Description: "d", Kind: apigen.ChallengeKind("static"), Type: "standard", TypeConfig: map[string]string{}, Flag: "f", Scoring: apigen.ScoringMode("static"), PointsInitial: 100, MemLimitMb: 0, CpuMillis: 0, ContainerEnv: map[string]string{}, Instancing: apigen.Instancing("none"), FlagMode: apigen.FlagMode("static"), WritablePaths: []string{}, Attachments: []apigen.AttachmentAdmin{}, Solves: 0, CurrentPoints: 100, CreatedAt: goldenTime, UpdatedAt: goldenTime}},
