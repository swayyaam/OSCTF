...
- **A CI step that stops exercising its target must fail, not pass quietly** — the same failure mode as the five integration tests that a `-run` name filter once silently dropped. `go test -run X` and `vitest --passWithNoTests` both exit 0 when nothing matched, and a solution-style `tsc --noEmit` (`files: []` + `references`) type-checks *nothing* in non-build mode — all three have masked an absent check here. So: select tests by tag/package, not name; assert the target actually ran (the soak step greps its own `--- PASS: TestSoak`); keep `typecheck` non-vacuous and pointed at a config that still covers test files (`tsconfig.test.json`) after they are excluded from the shippable build. The shippable artifact is proven only by the `image` + `smoke` jobs, which build in the **isolated** Docker context; the `web` job builds with the full repo checked out, so a green `web` does *not* prove the image builds — it once stayed green for a whole release cycle while the Docker build was broken. Because that cycle lived entirely in local, unpushed commits, CI never ran it: **cut a release only after `make smoke` (image build + compose smoke) passes locally**, not on a green `web` alone.
