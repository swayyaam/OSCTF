...
- **A CI step whose success is compatible with having done nothing is not a check — it is a no-op that reports green.** Name it and watch for it: this single failure mode is behind the five integration tests a `-run` filter once silently dropped *and* three of the four gaps the CI audit found, and it is **not specific to Go tests**. Any step passes vacuously when its selector matches nothing — `go test -run X` and `vitest --passWithNoTests` exit 0 on zero matches, a solution-style `tsc --noEmit` (`files: []` + `references`) type-checks *nothing* in non-build mode, and a lint or scan over an empty file set is clean. **Every new CI step must be checked against one question: could it pass while exercising nothing? If yes, make it assert it did something.** Select by tag/package, not name; fail on zero tests/files (no `--passWithNoTests`); assert the target actually ran (the soak step greps its own `--- PASS: TestSoak`); keep `typecheck` non-vacuous and covering test files (`tsconfig.test.json`) after they are excluded from the shippable build. And the artifact itself is proven only by the `image` + `smoke` jobs, which build in the **isolated** Docker context; the `web` job builds with the full repo checked out, so a green `web` does *not* prove the image builds — it once stayed green for a whole release cycle while the Docker build was broken. Because that cycle lived entirely in local, unpushed commits, CI never ran it: **cut a release only after `make smoke` (image build + compose smoke) passes locally**, not on a green `web` alone.
- **Compile every build tag before reporting green — an untagged or `-tags integration` run is not "the build."** This is the same no-op class as above: a local gate that never compiles the `//go:build dockerint` and `//go:build soak` files has a success compatible with those files being broken. A P1-auth wiring change (`Deps.Auth` → a registry) passed `go test` (unit) and `go test -tags integration` locally but turned CI red, because the one missed call site lived in `internal/soak/soak_test.go` (`//go:build soak`) — which neither local command compiled, and a scoped `golangci-lint` over three packages skipped too. The pre-report gate is **`make test`** (runs `make vet-tags` first) or `make vet-tags` directly; CI's `api-lint` (golangci-lint `build-tags: [integration, dockerint, soak]`) is the authoritative all-tags compile. Never report green off a partial-tag run. **`vet-tags` also guards its own tag list:** it greps every `//go:build` tag in the tree and fails if one is covered by neither `VET_TAGS` nor `TAG_ALLOWLIST` (e.g. `embed_spa`, excluded because it embeds the built SPA) — so a fifth build tag added later can't silently reintroduce the partial-compile gap. Same shape as the policy-table route-coverage gate: the check derives its expectation from the tree, not a hand-maintained list.
